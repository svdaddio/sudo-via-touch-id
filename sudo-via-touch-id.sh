...
if [[ "$EXIT" == "0" ]]; then #; if that code was zero, the file does not need to be modified
	echo "$NAME: '$FILE' already has correct entry."
else #; if that code was not zero, we'll try to modify that file
	...
	if [[ ${firstline:0:1} == '#' ]]; then #; Add first comment line, prior to $NEWTEXT
		echo "$firstline" >| "$TEMPFILE"
		echo "$NEWTEXT" >> "$TEMPFILE"
	else #; Add first line that was not a comment after $NEWTEXT
		echo "$NEWTEXT" >| "$TEMPFILE"
		echo "$firstline" >> "$TEMPFILE"
	fi
	...
	#; set the proper permissions/ownership and move the file into place
